...
export const generateFirstMessages = internalAction({
  ...
  handler: async (
    ctx,
    { fileUrl, courseId, storageId, feedbackId, weekNumber },
  ) => {
...
export const generateFeedback = mutationWithAuth({
  ...
  handler: async (ctx, { courseSlug, storageId, name, weekNumber }) => {
