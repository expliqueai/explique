...
export const insertMessage = internalMutation({
  ...
  handler: async ({ db }, { chatId, assistant, content, appearance }) => {
    return await db.insert("chatMessages", {
      chatId,
      content,
      assistant,
      appearance,
...
export const answerChatCompletionsApi = internalAction({
  ...
  handler: async (ctx, { chatId, userMessageId, assistantMessageId }) => {
...
export const sendMessage = mutationWithAuth({
  ...
  handler: async (ctx, { chatId, message }) => {
    const attempt = await getAttemptIfAuthorized(ctx.db, ctx.session, chatId);
